...
//a menos que en esta clase se redefinan ciertos metodos
    ...
    public function getTodos() {
        $lista = parent::getTodos();
        if (isset($lista->mensaje)) {
            return $lista;
        } else {
            $e = new Categoria();
            $entidad = $e->findById($lista->idCategoryPlate);
            if (isset($entidad->mensaje)) {
                $v = new Error();
                $v->mensaje = $todos->mensaje;
                $v->renderInclude();
            } else {
                $lista->categoria = $entidad->description;
                return $lista;
            }
        }
    }
    ...
    public function update($usuario) {
        ...
        try {
            $respuesta = file_get_contents($this->getUrl() . '/api/user/' . $id, false, $context);
            var_dump($respuesta);
            return $respuesta;
